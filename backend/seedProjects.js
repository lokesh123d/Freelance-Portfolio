...
async function seedProjects() {
    try {
        ...
        if (!db) {
            console.error('❌ Firebase database not initialized. Make sure serviceAccountKey.json exists.');
            process.exit(1);
        }
        ...
        if (snapshot.exists()) {
            console.log('⚠️  Projects already exist in database. Clearing old projects...');
            await projectsRef.remove();
        }
